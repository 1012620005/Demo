            ...
            <template slot-scope="scope">
              <div class="table-end-caozuo">
                <el-tooltip :enterable="false" :content="'点击启用'" placement="top">
                  <el-switch
                    @change="enableOption(scope.row.id, scope.row.login_flag)"
...
export default {
      ...
      this.$confirm("确认删除?", "提示", {
        ...
        beforeClose: (action, instance, done) => {
          if (action === "confirm") {
            ...
            let params = {ids:row.id, type:'1', userType:3};
  ...
  watch: {
    dead:function(){
      if(this.dead !== null){
        this.page.pageNo = this.dead.pageNo;
        this.page.pageSize = this.dead.pageSize;
        this.page.count = this.dead.count;
        this.stuList = this.dead.list;
      }
    }
  }
};
...
.dead-status .demo-table-expand .el-form-item {
margin-right: 0;
margin-bottom: 0;
width:33.3%;
}
