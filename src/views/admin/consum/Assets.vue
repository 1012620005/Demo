          ...
          <div style="margin:0 0 20px;">
            <el-table
              ...
              <el-table-column
                ...
                width="100">
                <template slot-scope="scope">
                  <span v-if="scope.row.status == '4'">有库存</span>
                  <span v-if="scope.row.status == '5'">无库存</span>
              ...
              <el-table-column
                ...
                >
                <template slot-scope="scope">
                  <el-button v-if="scope.row.status == '4'" @click="chenge(scope.row)" type="text" size="small">修改</el-button>
                  <el-button  @click="ckDetailed(scope.row)" type="text" size="small">出库明细</el-button>
                  <el-button v-if="scope.row.status == '4'" @click="deleten(scope.row)" type="text" size="small">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
...
<el-dialog  :visible.sync="dialog_ck_created"  width="30%" >
  ...
  <div style="max-width:420px; text-align:left;">
    <el-form :model="ck_form"  :label-position="labelPosition"  label-width="110px"  ref='form_ck'>
        ...
        <el-form-item  label="借记人:" :rules="{
          required: true, message: '借记人不能为空', trigger: 'blur'
        }" prop="name" >
          <el-input v-model="ck_form.name"    placeholder="">
          </el-input>
        </el-form-item>
        <el-form-item  label="借记原因:"  :rules="{
          required: true, message: '借记原因不能为空', trigger: 'blur'
        }"  prop="reason" >
          <el-input
...
export default {
  ...
  methods: {
    import_asste(){
      AD.import_asste({'type':"2","status":""}).then(r=>{
        if(r){
          this.$message({
            showClose: true,
            message: '导出成功',
    ...
    click_ck_created(){
      if(this.def_select !== ""){
        this.dialog_ck_created = true;
          this.ck_form.name="";
          this.ck_form.department="";
          this.ck_form.reason="";
          this.ck_form.returnDate="";
          this.ck_form.assetsId="";
          this.ck_form.AssetsType="2";
          this.ck_form.number=null;
      }else{
        this.$message({
          showClose: true,
          message: '请选择出库物品',
