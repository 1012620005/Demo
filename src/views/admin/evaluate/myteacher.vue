<template>
  <div class="stuestTch">
        <div class="stuestTch-top clear" >
            <el-row :gutter="20">
              <el-col  :span="18">
                <el-form :inline="true" :model="select_form" label-width="75px"  >
                    <el-form-item label="评价类别">
                      <el-select v-model="select_form.km" clearable  @change="getre" >
                          <el-option
...
<el-dialog  :visible.sync="dialogmap"  width="45%" >
  <div slot="title"  >
      <p style="font-size:16px;" @click="getimg" >雷达图</p>
  </div>
...
export default {
  ...
  methods: {
    ...
    getlist(){
      let u={
        'xq':this.select_form.xq.term,
        'type':this.select_form.km,
        'pageSize':10,
        'pageNo':this.page.pageNo,
      }
      AD.myevaluate(u).then(r=>{
        if(r){
    ...
    getimg(){
      let canvas = this.$refs.map.children[0].children[0]
      console.log(canvas)
