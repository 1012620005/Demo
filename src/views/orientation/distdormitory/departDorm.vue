<template>
	<div class="contanier">
        ...
        <!--主体内容-->
        <el-row :span="24" class="depdorm-main">
            <el-col :span="24" class="depdorm-top">
                <el-form :inline="true" class="demo-form-inline">
                    <el-form-item label="校区">
                        <el-select v-model="campus" value-key="id" clearable placeholder="请选择校区" @change="selectCampus">
                            <el-option :label="item.name"  v-for="(item,index) in campusList " :value="item" :key="index">{{item.name}}</el-option>
                        </el-select>
                    </el-form-item>
                </el-form>
            </el-col>
            <el-col :span="24" class="depdorm-table">
                <el-table
                    :data="tableData"
                    border
                    v-loading="loading"
                    ...
                    <el-table-column label="操作" width="200">
                        <template slot-scope="scope">
                            <div @click="distrDormForDept(scope.row)" v-if="scope.row.state == 1"  class="word handle clearfix">
                                <span class="imgScript refresh"></span>
                                <P>待分配</P>
                            </div>
                            <div v-if="scope.row.state == 2" class="word">
                                <span class="imgScript shenhe"></span>
                                <p>待审核</p>
                                <span @click="readDistrInfo(scope.row)"  class="imgScript seeBtn"></span>
                            </div>
                            <div v-if="scope.row.state == 3" class="word">
                                <span class="imgScript success"></span>
                                <p>已通过</p>
                                <div class="derivation"><a :href=" exportExcelURL + scope.row.deptId" class="clearfix handle"><span class="imgScript exporticon"></span>导出</a></div>
                                <span @click="readDistrInfo(scope.row)"  class="imgScript seeBtn"></span>
                                
                            </div>
                            <div v-if="scope.row.state == 4" class="word">
                                <span class="imgScript nosussess"></span>
                                <p>未通过</p>
                                <el-tooltip class="item" effect="dark" content="重新分配" placement="bottom">
                                    <span @click="distrDormForDept(scope.row)"  class="imgScript btns"></span>
                                </el-tooltip>
                                <span @click="readDistrInfo(scope.row)"  class="imgScript seeBtn"></span>
                            </div>
                        </template>
        ...
        <!--宿舍分配详情弹框-->
        <el-dialog class="disdorm-deta" :visible.sync="showDisdormdeta">
...
<script type="text/javascript">
    ...
    import api from '../../../apis/orientation/distdormitory/departDorm.js'
	...
	export default {
        ...
        methods:{
            ...
            readDistrInfo(row){
                ...
                this.distrDorm = row;
                api.getDistrDormByMainId(row.deptId).then(response =>{
            ...
            selectCampus(val){
                api.distrTotalForDept(val.id).then(response =>{
                    var mydata = response.data.data;
                    var meta = response.data.meta;
                    if(meta.code == '200')
        ...
        mounted () {
            ...
            api.distrTotalForDept().then(response =>{
                var mydata = response.data.data;
                var meta = response.data.meta;
                if(meta.code == '200')
                    this.showData(mydata)
...
<style type="text/css" lang="scss" scoped>
    ...
    .depdorm-main{
        ...
        .depdorm-table{
            ...
            .refresh,.shenhe,.success,.nosussess,.exporticon,.btns{
                float: left;
                margin-top: 4px;
            }
            .btns{
