<template>
	<div class="contanier">
        ...
        <!--主体内容-->
        <el-row :span="24" class="classdorm-main">
            <el-col :span="24" class="classdorm-top">
                <el-form :inline="true"  class="demo-form-inline">
                    <el-form-item label="校区">
                        <el-select v-model="campus" value-key="id" placeholder="" @change="selectCampus">
                            <el-option v-for="item in campusList" :label="item.name" :value="item" :key="item.id">{{item.name}}</el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="院系">
                        <el-select v-model="dept" value-key="id" placeholder="" @change="selectDept">
                            <el-option v-for="item in deptList" :label="item.name" :value="item" :key="item.id">{{item.name}}</el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="专业">
                        <el-select v-model="prof" value-key="id" placeholder="" @change="selectProf">
                            <el-option v-for="item in profList" :label="item.name" :value="item" :key="item.id">{{item.name}}</el-option>
                        </el-select>
                    </el-form-item>
                </el-form>
            </el-col>
            <el-col :span="24" class="classdorm-table" >
                <template>
                    <el-table :data="tableData" style="width: 100%" border>
                        ...
                        <el-table-column prop="clazzName" label="班级" show-overflow-tooltip></el-table-column>
                        ...
                        <el-table-column prop="createDate" label="提示日期" show-overflow-tooltip></el-table-column>
                        <el-table-column label="操作" width="150">
                            <template slot-scope="scope">
                                <div v-if="scope.row.state == 1" @click="distrDormForClazz(scope.row)"class="word handle clearfix">
                                    <span class="imgScript refresh"></span>
                                    <P>待分配</P>
                                </div>
                                <div v-else-if="scope.row.state == 2" class="word" >
                                    <span class="imgScript shenhe"></span>
                                    <p>待审核</p>
                                    <span class="imgScript seeBtn" @click="readDistrInfo(scope.row)"></span>
                                </div>
                                <div v-else-if="scope.row.state == 3 || scope.row.state == 5" class="word">
                                ...
                                <div v-else-if="scope.row.state == 4 " class="word">
                                    <span class="imgScript nosussess" @click="distrDormForClazz(scope.row)"></span>
                                    <p>未通过</p>
                                    <span class="imgScript seeBtn" @click="readDistrInfo(scope.row)"></span>
                                </div>   
                            </template>
            ...
            <div class="disdorm-bot">
                <div class="ddt-top">
                    <el-breadcrumb separator=" ">
                        ...
                        <el-breadcrumb-item>{{distrDorm.clazzName}}</el-breadcrumb-item>
...
<script type="text/javascript">
    ...
    import api from '../../../apis/orientation/distdormitory/classDorm.js'
	...
	export default {
        ...
        methods:{
            ...
            selectCampus(){
                ...
                api.loadSelect(params).then(response =>{
                ...
                api.distrTotalForClazz(params).then(response =>{
                    var meta = response.data.meta
                    var mydata = response.data.data;
                    if(meta.code == '200') {
            ...
            // exportExcelDistrDorm(row){
            //     var params = {
            //         mainRelation : row.clazzId
            //     }
            //     api.exportExcelDistrDorm(params).then(response =>{
            //         var mydata = response.data.data;
            //         var meta = response.data.meta;
            //         if(meta.code == '200'){
            //             this.window.boy = mydata.boy;
        ...
        mounted () {
            ...
            }),//'0761de6d4a1c43cc9a39180222cecd8a','d82de0f42f6b4187b1bdd1c8928877b2c','7088ebaa66d7471ebc052e2b785a1df74'
            api.distrTotalForClazz({}).then(response =>{
                var meta = response.data.meta
                var mydata = response.data.data;
                if(meta.code == '200') {
                    this.showData(mydata);
