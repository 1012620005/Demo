...
export default {
  ...
  created() {
    course.queryuser({ id: this.$Cookies.get("user") }).then(r => {
      ...
      if (this.user.course.length > 0) {
        this.flagclazz = true;
        this.defa.bj = this.user.course;
        this.form.bj = this.user.course[0].clazzid;
      }
      this.getkb();
    });
  },
  ...
  methods: {
    ...
    cellclick(row, column, cell, event){
      if(column.property == "tablemorning"){
        ...
        console.log(row)
        console.log(column)
        console.log(row.Schedule[column.property])
        console.log(event)
        let dome = row.Schedule[column.property]
        if(row.Schedule.isPublic == '1' || dome.valueIndex == null){
          return
        }else{
          let u={
            'teachId':this.$Cookies.get('userId'),
            'classId':dome.str2,
            'someDay':dome.someDay,
            'secIndex':dome.sectionIndex,
            'startTime':dome.startTime,
            'endTime':dome.endTime
          }
          course.get_code(qs.stringify(u)).then(r=>{
            this.dialogVisibletime = true
            if(r){
              this.src = r.data.data
            }
          })
        }
      }
    ...
    querykbweek() {
      ...
      let yearsweek = (((new Date(this.form.data))-(new Date(this.years)))/(24*60*60*7*1000)|0) +1; 
      console.log(this.form.fewWeek)
      if(this.form.data !== ''){
      ...
      course.querykb(par).then(r => {
        ...
        if (r) {
          if (r.data.data !== null) {
            ...
            this.years = startTime.split('-')[0]+'-01-01';
            this.startweekofyear = (((new Date(startTime))-(new Date(this.years)))/(24*60*60*7*1000)|0) +1;
