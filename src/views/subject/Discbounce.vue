<template>
    <div class="sfaBounce">
        <div class="sfaBounce-td clearfix">
            <span class="add">添加学科</span>
            <img class="close" @click="closeBounce()" src="../../assets/img/overtime/close.png" alt="">
        </div>
        <el-form :model="ruleForm"  ref="ruleForm" label-width="93px" class="demo-ruleForm clearfix" style="padding:22px 20px 0 20px;">
            <div v-for="(item,index) in number" :key="index" >
                <el-form-item label="学科名称：" :prop="'subjectNameVO['+index+']'" :rules="{required: true, message: '请输入学科名称', trigger: 'blur'}"> 
                    ...
                    if (valid) {
                        ...
                        if(!this.ruleForm.id) {
                            this.$confirm('确认提交吗？', '提示', {}).then(() => {
                                // this.number = '1'
                                api.postDisc(datax).then(response=>{
